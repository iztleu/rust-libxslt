...
impl Stylesheet {
    ...
    pub fn transform(
        ...
        let params_cstrings_result: Result<Vec<CString>, _> = params
            .iter()
        ...
        let mut params_cstrings_pointers: Vec<*const libc::c_char> =
            params_cstrings.iter().map(|cstr| cstr.as_ptr()).collect();
    ...
    fn build_context(&mut self, doc: &Document) -> Result<TransformContext, &'static str> {
        unsafe {
            // let ptr = catch_unwind(move || { xsltNewTransformContext(self.ptr(), doc.doc_ptr()) }).unwrap(); //
